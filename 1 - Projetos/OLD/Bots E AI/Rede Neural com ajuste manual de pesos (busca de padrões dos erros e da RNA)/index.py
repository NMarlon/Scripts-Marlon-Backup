...
class RNA:    
    ...
    def disparo(tipo, entrada):
        ...
        if(tipo == "relu" or tipo == 0):
            if entrada<0:
                return 0#saida
            else:
                return entrada
        elif (tipo == "identity" or tipo == "indentico" or tipo == 1):
            return entrada
        elif (tipo == "sig" or tipo == "sigmoide"or tipo == "sigmoid" or tipo == 2):
            return 1/(1+(e**(-entrada)))
        elif (tipo =="tan" or tipo=="tangente" or tipo == 3):
            return ((e**entrada)-(e**(-entrada)))/((e**entrada)+(e**(-entrada)))
        elif (tipo=="gelu" or tipo == 4):
            error("Função ainda não está pronta! pegando a funcao 'identica'")
            return entrada#(entrada*(1+e))/2
        elif (tipo=="softplus" or tipo == 5):
            return math.log(1 + (e**entrada),10)
        elif (tipo == "elu" or tipo == 6):
            if(entrada <= 0):
                return self.alfa*((e**entrada)-1)
            else:
                return entrada
        elif (tipo == "selu" or tipo==7):

            error("Função ainda não está pronta! pegando a funcao 'identica'")
            return entrada#(entrada*(1+e))/2
        elif(tipo == "lrelu" or tipo =="leakyrelu" or tipo==8):
            if(entrada<0):
                return 0.01*entrada
            else:
                return entrada
        elif(tipo == "prelu" or tipo == 9):
            if (entrada<0):
                return self.alfa*entrada
            else:
                return entrada
        elif(tipo=="silu" or tipo == 10):
            return entrada/(1+(e**(-entrada)))
        elif(tipo=="gaussiana" or tipo=="gaussian"or tipo=="gaus" or tipo==11):
            return e**((-entrada)**2)         
