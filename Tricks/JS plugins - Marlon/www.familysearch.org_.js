/*//www.familysearch.org_.js

A ideia é ser um facilitador



*/

//------------------------- TRECHO ASSIST DEVELOP - ---------- START
//v0.2.3
...
//Qualquer modificação de algo daqui, por favor acesse o arquivo origninal no meu Gist: https://gist.github.com/7901169778902334d73facad9c864f0d
//------------------------- TRECHO ASSIST DEVELOP - ---------- END
...
window.onload = (start=>{console.log("Work!");
  ijct_script('https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js');
});


id_num_Certidao_get_cookie_and_change_field=(()=>{
  if(getCookie('Num_Certidao_last')==''){
    setCookie('Num_Certidao_last',$(id_num_Certidao).html(),dias_manter_cookies_patch);
  }else{
    $(id_num_Certidao).ready(()=>{
      $(id_num_Certidao).html(getCookie('Num_Certidao_last'));        
    });
  } 
  ExReg("teste_4",7,"4: ");
  
});
  new_register=((id_tipo)=>{
    setCookie('Tipo_registro_last',$(id_num_Certidao).html(),dias_manter_cookies_patch);
  }); 
id_num_Certidao_onchange=(()=>{
  id_num_Certidao_get_cookie_and_change_field();
  
  $(id_num_Certidao).on('DOMSubtreeModified', (()=>{setCookie('Num_Certidao_last',$(id_num_Certidao).html(),dias_manter_cookies_patch)}));
...
nascimento=(()=>{  
  ...
  $(id_num_Certidao).on('DOMSubtreeModified', (()=>{setCookie('Num_Certidao_last',$(id_num_Certidao).html(),dias_manter_cookies_patch);id_num_Certidao_onchange();}));
  id_num_Certidao_get_cookie_and_change_field();
});
obito=(()=>{
...
config_on_start=()=>{
  if($(id_tipo_de_registro).html()=="Nascimento"){
    nascimento()
  }else if($(id_tipo_de_registro)=="Óbito"){
    obito()
  }else if($(id_tipo_de_registro)=="Casamento"){
    casamento()
  }
...
window.onload=(()=>{
  ...
  var dias_manter_cookies_patch=120;//por quanto tempo os cookies vão ficar salvos
  var reset_cookies_onload=false;
  if(reset_cookies_onload){
    setCookie('Num_Certidao_last','',- 1);//reset cookies ao abrir a página
  }
  var salvar_cookies=true;
  //Colocar em todos os "setCookies" o if(salver_cookies)
  if(getCookie('usar_cookies')!="true"){
    if(salvar_cookies)
      setCookie('usar_cookies',salvar_cookies,365000);
    
  }
  var salvar_cookies=!!getCookie('usar_cookies');
  var puxar_numero_da_certidao = true;
  ExReg("teste_2",7,"2: ");


  var id_reg_atual_tipoDeRegistro="00";
  var id_tipo_de_registro="#"+"entryFieldformEntry"+id_reg_atual_tipoDeRegistro;

  var id_reg_atual_numCertidao="000";
  var id_num_Certidao="#"+"entryFieldformEntry"+id_reg_atual_numCertidao; 
  var id_dia_do_registro="#"+"entryFieldformEntry00"+(parseInt(id_reg_atual_numCertidao)+1).toString();

  config_on_start();

  $(id_tipo_de_registro).on('DOMSubtreeModified', (()=>{}));
});
